...
| `bin`      | `~/.local/bin/dclaude` (+ `deepseek-code` alias) | Claude Code routed to DeepSeek, separate config in `~/.claude-deepseek`. Needs `export DEEPSEEK_API_KEY=...` first. |
| `claude-settings` | copied to `~/.claude/settings.json` and `~/.claude-deepseek/settings.json` | Copied, not linked (Claude Code rewrites it). After changing settings, copy back: `cp ~/.claude/settings.json ~/dotfiles/claude-settings/`. |
